...
ahrsBringup::ahrsBringup() :frist_sn_(false), serial_timeout_(20)
{
  ...
  ori_imu_pub_ = ori_nh_.advertise<fdilink_ahrs::original_imu>("/bicycle/fdi/ori_imu", 1000);
  //setp up serial
  try
  {
...
void ahrsBringup::processLoop()
{
  ...
  while (ros::ok())
  {
    ...
    if (head_type[0] == TYPE_AHRS)
    {
      ...
      fdilink_ahrs::original_imu ori_imu_data;
      ...
      if (device_type_ == 0)         //未经变换的原始数据
      {
        ...
        ori_imu_data.gyroscope_x = imu_frame_.frame.data.data_pack.gyroscope_x;
        ori_imu_data.gyroscope_y = imu_frame_.frame.data.data_pack.gyroscope_y;
        ori_imu_data.gyroscope_z = imu_frame_.frame.data.data_pack.gyroscope_z;
        ori_imu_data.accelerometer_x = imu_frame_.frame.data.data_pack.accelerometer_x;
        ori_imu_data.accelerometer_y = -imu_frame_.frame.data.data_pack.accelerometer_y;
        ori_imu_data.accelerometer_z = -imu_frame_.frame.data.data_pack.accelerometer_z;
      }
      ...
      ori_imu_pub_.publish(ori_imu_data);
    } 
  }
}
